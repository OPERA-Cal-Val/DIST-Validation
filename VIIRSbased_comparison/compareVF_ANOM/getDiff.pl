...
$layer = $ARGV[0];
$outpath = "VF_3.5.1/Diff$layer"; if (!-d $outpath){mkdir"$outpath";}
...
my @list :shared;
open(DAT,"newHLS.txt") or die "file not found\n"; @list = <DAT>; foreach(@list){chomp;}
#@list =@list[0..1];
#if(substr(0,3,$layer) eq "GEN"){compileGEN();}
#else{compile();}
compileGEN();
#push(@serverlist, "17,20");
push(@serverlist, "23,20");
...
sub ExpTh {while($HLS_ID = shift(@list)){
  ...
  $newpath = "VF_3.5.1/VIIRS";
  $oldpath = "/gpfs/glad3/HLSDIST/testing/v1sample/DIST-ALERT_v1/$year/$tilepathstring/$DIST_ID";
  if(-e "$newpath/$DIST_ID\_$layer.tif"){
    if(-e "$oldpath/$DIST_ID\_$layer.tif"){
      ...
      system"ssh gladapp$server \'cd $currdir; ./diff$layer $DIST_ID $newpath $oldpath $zone\'";
