...
sub ExpTh {while($tile = shift(@list)){
  ...
  $newpath = "DIST-ANN_v1_VIIRS_/$tilepathstring/2022h";
  $oldpath = "DIST-ANN_v1_current_/$tilepathstring/2022h";#/gpfs/glad3/HLSDIST/testing/v1sample/
  $newfile = readpipe"ls $newpath/OPERA*$layer.tif";chomp($newfile);
  $oldfile = readpipe"ls $oldpath/OPERA*$layer.tif";chomp($oldfile);
  if(-e "$newfile"){
    if(-e "$oldfile"){
      #print"\n\nssh gladapp$server \'cd $currdir; ./diffMat$layer $newfile $oldfile $zone $tile\'\n\n";
