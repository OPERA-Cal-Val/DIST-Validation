...
$layer = $ARGV[0];
$outpath = "Diff__$layer"; if (!-d $outpath){mkdir"$outpath";}
...
my @list :shared;
open(DAT,"samefiles.txt") or die "file not found\n"; @list = <DAT>; foreach(@list){chomp;}
#@list =@list[0..1];
#compile();
#compile16();
compileMatrix();
...
sub ExpTh {while($HLS_ID = shift(@list)){
  ...
  $newpath = "DIST-ALERT_v1_VIIRS_/$year/$tilepathstring/$DIST_ID";
  $oldpath = "DIST-ALERT_v1_current_/$year/$tilepathstring/$DIST_ID";#/gpfs/glad3/HLSDIST/testing/v1sample/
  if(-e "$newpath/$DIST_ID\_$layer.tif"){
    if(-e "$oldpath/$DIST_ID\_$layer.tif"){
      #print"ssh gladapp$server \'cd $currdir; ./diff$layer $DIST_ID $newpath $oldpath $zone\'\n";
      system"ssh gladapp$server \'cd $currdir; ./diffMat$layer $DIST_ID $newpath $oldpath $zone\'";
    }else{print"missing $oldpath/$DIST_ID\_$layer.tif\n";}
  }else{print"missing $newpath/$DIST_ID\_$layer.tif\n";}
}}
...
ofstream outhist;
outhist.open(\"$outpath/\"+DISTID+\".txt\");
