...
$outscene = $ARGV[1];
$output = $ARGV[2];
$currentsource = $ARGV[3];

$HLSsource = "/gpfs/glad3/HLS";
#$currentsource = "/gpfs/glad3/HLSDIST/VIIRStest/lasrc_timeSeries/VIIRS";
...
$band = $bands[0];
print OUT"
filename=\"$currentsource/$sensor/$year/$tilepathstring/$im1/$im1.$band.tif\";
...
$b = 1;
  foreach $band (@bands[1..3]){
    print OUT"
filename = \"/gpfs/glad3/HLSDIST/VIIRStest/lasrc_timeSeries/VIIRS/$sensor/$year/$tilepathstring/$im1/$im1.$band.tif\";
INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); 
INBAND = INGDAL->GetRasterBand(1);
...
filename=\"$currentsource/$sensor/$year/$tilepathstring/$im1/$im1.Fmask.tif\";
INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); 
INBAND = INGDAL->GetRasterBand(1);
