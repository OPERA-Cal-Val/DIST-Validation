...
def runGranule(server,granule,mode="ALL"):
  ...
  if mode == "VEG_IND":
    ...
    if not os.path.exists(outdir+"/"+DIST_ID+"_GEN-ANOM.tif"):
      ...
      #print("ssh gladapp"+server+" \'cd "+currdir+"; perl 02C_GEN_ANOM.pl "+granule+" "+DIST_ID+" "+outdir+" "+HLSsource)
      response = subprocess.run(["ssh gladapp"+server+" \'cd "+currdir+"; perl 02C_GEN_ANOM.pl "+granule+" "+DIST_ID+" "+outdir+" "+HLSsource+" 2>>errorLOG.txt\'"],capture_output=True,shell=True)
      Errors = Errors + str(response.stderr.decode())#.split('\n')[-1]
